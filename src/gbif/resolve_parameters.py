...
# Check if any alternative's canonical name matches the search name
            ...
            if result.get("usage") and result.get("usage", {}).get("key"):
                ...
                continue
                            ...
                            # Parent taxon fallback for intermediate ranks not in backbone
                            parent_name = organism.parent_scientific_name
                            if parent_name:
                                await process.log(
                                    f"'{name}' ({rank}) is not in the GBIF Backbone Taxonomy. "
                                    f"Resolving parent taxon '{parent_name}' instead."
                                )
                                ...
                                if parent_result.get("usage") and parent_result.get("usage", {}).get("key"):
                                    ...
                                    await process.log(
                                        f"Resolved parent taxon '{parent_name}' to key {parent_key}. "
                                        f"Results will include all {parent_name} occurrences filtered by text mention of '{name}'. "
                                        f"Note: this is an approximation — some {name} records may be missed if they don't mention the name explicitly.",
                                        data={"url": parent_url},
                                    )
                                    await process.create_artifact(
                                        mimetype="application/json",
                                        description=f"GBIF Species Match for parent taxon: {parent_name} (fallback from {name})",
                                        uris=[parent_url],
                                        metadata={
                                            "data_source": "GBIF Species Match",
                                        },
